...
// Consulta para buscar o dono e todos os funcionários
$sql_dono = "SELECT id, nomeCompleto AS nome FROM usuarios WHERE cargo = 'dono'";
...
<div class="col-md-4 offset-md-4">
    <form method="post" action="proc_cadastro_servicos.php" id="proc_cadastro_servicosForm">
        ...
        <div class="form-group">
            <?php
            // Exibir o dono como checkbox
            if ($dono) {
                echo '<div class="form-check">';
                echo '<input class="form-check-input" type="checkbox" name="funcionarios[]" value="' . $dono['id'] . '" id="funcionario_dono">';
                echo '<label class="form-check-label" for="funcionario_dono">' . $dono['nome'] . ' (Dono)</label>';
                echo '</div>';
            }
            ...
            if ($result_funcionarios->num_rows > 0) {
                while ($row = $result_funcionarios->fetch_assoc()) {
                    echo '<div class="form-check">';
                    echo '<input class="form-check-input" type="checkbox" name="funcionarios[]" value="' . $row['id'] . '" id="funcionario_' . $row['id'] . '">';
